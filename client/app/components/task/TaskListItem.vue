<template>
  <div class="group relative flex items-center p-3 rounded-sm bg-gray-50 hover:bg-gray-100 transition-colors duration-200">
    ...
    <div class="flex-1 min-w-0">
      <p class="text-sm transition-all duration-200" :class="[task.completed ? 'text-gray-500 line-through' : 'text-gray-900']">
        {{ task.title }}
      </p>
      <p v-if="task.description" class="text-xs mt-1 transition-all duration-200" :class="[task.completed ? 'text-gray-400 line-through' : 'text-gray-600']">
        {{ task.description }}
      </p>
    </div>
    ...
    <div class="flex-shrink-0 ml-3 h-6 relative flex items-center">
      ...
      <div v-if="task.createdAt" class="absolute inset-0 flex items-center justify-end text-xs text-gray-400 transition-opacity duration-200 opacity-100 group-hover:opacity-0">
